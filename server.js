        ...
        switch (tipo) {
            case 'cadastro':
                var [rows, fields] = await pool.query(
                    "insert into `freedb_pedromysql`.`tbl_login` (`nome`, `login`, `senha`) values (?, ?, ?);",
                    [nome, login, senha]
                );
                ...
                case 'login':
                    try {
                        var [rows, fields] = await pool.query(
                            "select * from `freedb_pedromysql`.`tbl_login` where `nome` = ? and `login` = ? and `senha` = ?;",
                            [nome, login, senha]
                        );
                        if (rows.length >= 1) {
                ...
                var strSql = "select * from `freedb_pedromysql`.`tbl_login` where" + 
                ...
                break;
                case 'atualizacao':
                    var strSql = "select * from `freedb_pedromysql`.`tbl_login`;";
                    var [rows, fields] = await pool.query(strSql);
                    if (rows.length > 0) {
                        res.json({ 
                ...
                case 'atualizar':
                    ...
                    var strSql = "update `freedb_pedromysql`.`tbl_login` set " + 
